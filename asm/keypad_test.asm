; KEypad Test Program
; This program tests the keypad by reading the keys and displaying them on the serial port
...
KEYPAD_ADDRESS EQU 0x60 ; Keypad Columns address
KEYPAD_COLUMNS EQU 0x03 ; number of columns
KEYPAD_ROWS EQU 0x05 ; number of rows
KEYPAD_MODIFIERS EQU 0x03 ; modifiers columns
KEYPAB_BUFFER_LENGTH EQU 0x04 ; buffer length for keys

Main:
    LD HL, KEYPAD_StartMSG
    CALL PrintString ; Print the start message
.loop:
    CALL KEYPAD_Scan ; Scan the keypad Columns
    LD HL, KEYPAD_BUFFER ; Load the address of the buffer
    CALL PrintString
    LD HL, KEYPAD_BUFFER
...
.scanModifiers:
    IN A, (KEYPAD_ADDRESS+ KEYPAD_MODIFIERS) ; Read the modifier keys
    PUSH AF
    BIT 3, A
    JP Z, .noEnter
    PUSH AF
...
;    CALL SendChar_A ; display the key if not already set
    ...
    LD A, L
    SUB KEYPAD_BUFFER
    CP KEYPAB_BUFFER_LENGTH
    JP Z, .endPrintModifedChar
    INC HL ; Increment the buffer pointer
...
.continue:
    ...
    INC C
    LD A, C
    CP KEYPAD_ADDRESS + KEYPAD_COLUMNS ; Check if we have scanned all columns 
    JP NZ, .colLoop ; If not, continue scanning
    RET

; KEYPAD_DisplayKeys: ; Display the keys on the serial port
;     LD HL, COLS_DATA ; Load the address of rows data
;     LD B, KEYPAD_COLUMNS ; Number of columns to display
; .displayLoop:
;     LD A, (HL)
;     CALL Hex2Str ; Convert to hex and send to SIO port A
;     LD A, ' ' ; Send space to SIO port A
;     CALL SendChar_A
;     INC HL ; Move to next row data  
;     DJNZ .displayLoop ; Loop for all columns
;     RET

KEYPAD_delay:
    LD B, 0xFF ; Delay loop
.delayLoop:
    NOP ; No operation
    DJNZ .delayLoop ; Decrement B and loop if not zero
    RET


KEYPAD_StartMSG:
    DB "Keypad test program started", 0x0D, 0x0A, 0x00
...
KEYPAD_BUFFER: ; 4 charracters buffer
    DB 0x00, 0x00,0x00, 0x00, 0x00
